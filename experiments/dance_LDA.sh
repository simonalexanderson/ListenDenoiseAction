...
data_dir=data/dance_source/processed_sm0_0_30fps
...
do
	...
	do
		...
		python synthesize.py --checkpoints="${checkpoint}" --data_dirs="${src_dir}" --input_files="${input_file}" --styles="${style}" --start=${start} --end=${length} --seed=${seed} --postfix=${postfix} --trim=${trim} --dest_dir=${dest_dir} --gpu=${gpu} --video=${render_video} --outfile=${output_file}
